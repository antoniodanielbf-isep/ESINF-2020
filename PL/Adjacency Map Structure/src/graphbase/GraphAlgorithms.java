...
public class GraphAlgorithms {
    ...
    public static <V, E> Graph<V, E> kruskall(Graph<V, E> g) {
        Graph<V, E> mst = new Graph<>(true);
        Set<Edge<V, E>> edges = new TreeSet<>();
        LinkedList<V> connectedVerts = new LinkedList<>();

        for (V pos : g.allkeyVerts()) {
            mst.insertVertex(pos);
        }
        for (Edge<V, E> edge : g.edges()) {
            edges.add(edge);
        }
        for (Edge<V, E> edge : edges) {
            connectedVerts = GraphAlgorithms.DepthFirstSearch(mst, edge.getVOrig());
            if (!connectedVerts.contains(edge.getVDest())) {
                mst.insertEdge(edge.getVOrig(), edge.getVDest(), edge.getElement(), edge.getWeight());
            }
        }
        return mst;
    }
