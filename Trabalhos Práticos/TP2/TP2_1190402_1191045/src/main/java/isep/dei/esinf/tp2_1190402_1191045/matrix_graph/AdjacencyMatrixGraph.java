...
public class AdjacencyMatrixGraph<V, E> implements BasicGraph<V, E>, Cloneable {
    ...
    public Iterable<V> directConnections(V vertex) {
        ...
        List<V> directConnectedVertex = new ArrayList<>();
        for (int i = 0; i < edgeMatrix[vertices.indexOf(vertex)].length; i++) {
            if (edgeMatrix[vertices.indexOf(vertex)][i] != null) {
                directConnectedVertex.add(vertices.get(i));
            }
        }
        return directConnectedVertex;
    }
    ...
    public Iterable<E> outgoingEdges(V vertex) {
        ...
        for (int j = 1; j < edgeMatrix[vertices.indexOf(vertex)].length; j++) {
            if (edgeMatrix[vertices.indexOf(vertex)][j] != null) {
                outgoingEdges.add(getEdge(vertices.get(vertices.indexOf(vertex)), vertices.get(j)));
            }
        }
    ...
    public boolean removeVertex(V vertex) {
        ...
        for (int i = index; i < numVertices; i++) {
            System.arraycopy(edgeMatrix[i + 1], 0, edgeMatrix[i], 0, edgeMatrix.length);
        }
        ...
        for (int i = index; i < numVertices; i++) {
            for (E[] edgeMatrix1 : edgeMatrix) {
                edgeMatrix1[i] = edgeMatrix1[i + 1];
            }
        }

        for (E[] edgeMatrix1 : edgeMatrix) {
            edgeMatrix1[numVertices] = null;
        }
