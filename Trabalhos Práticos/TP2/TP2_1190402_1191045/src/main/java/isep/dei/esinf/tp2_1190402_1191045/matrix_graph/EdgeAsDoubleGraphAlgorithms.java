...
public class EdgeAsDoubleGraphAlgorithms {
    ...
    private static <V> void shortestPath(AdjacencyMatrixGraph<V, Double> graph, int sourceIdx, boolean[] knownVertices, int[] verticesIndex, double[] minDist) {
        ...
        minDist[sourceIdx] = 0;
        l.add(graph.vertices.get(sourceIdx));
        while (!l.isEmpty()) {
            sourceIdx = graph.vertices.indexOf(l.get(0));
            for (V v : l) {
                if (minDist[graph.vertices.indexOf(v)] < minDist[sourceIdx]) {
                    sourceIdx = graph.vertices.indexOf(v);
                }
            }
            knownVertices[sourceIdx] = true;
            l.remove(graph.vertices.get(sourceIdx));
            for (V adj : graph.directConnections(graph.vertices.get(sourceIdx))) {
                int vAdjIndex = graph.vertices.indexOf(adj);
                V vOrig = graph.vertices.get(sourceIdx);
                if (!knownVertices[vAdjIndex] && minDist[vAdjIndex] > minDist[sourceIdx] + graph.getEdge(vOrig, adj)) {
                    minDist[vAdjIndex] = minDist[sourceIdx] + graph.getEdge(vOrig, adj);
