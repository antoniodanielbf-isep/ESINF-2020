...
public class GraphAlgorithmsTest {
    ...
    @Test
    public void testTransitiveClosure() throws CloneNotSupportedException {
        System.out.println("Test of transitive closure graph");

        AdjacencyMatrixGraph<String, String> newMap = GraphAlgorithms.transitiveClosure(incompleteMap, "Virtual");

        // New map should have full connectivity in three islands (incomplete Map)
        // Faro is not connected to other cities
        assertTrue("Faro should have no connections", newMap.outDegree("Faro") == 0);

        // Viseu should be directly connected to Guarda and Castelo Branco
        assertTrue("Viseu should have 2 connections", newMap.outDegree("Viseu") == 2);

        Iterator<String> it = newMap.directConnections("Viseu").iterator();

        assertTrue("Viseu should be connected to Guarda", it.next().compareTo("Guarda") == 0);
        assertTrue("and Castelo Branco", it.next().compareTo("Castelo Branco") == 0);

        // Porto should be connected to 6 other cities
        assertTrue("Porto should have 6 connections", newMap.outDegree("Porto") == 6);

        it = newMap.directConnections("Porto").iterator();

        assertTrue("Porto should be connected to Braga", it.next().compareTo("Braga") == 0);
        assertTrue("and Vila Real", it.next().compareTo("Vila Real") == 0);
        assertTrue("and Aveiro", it.next().compareTo("Aveiro") == 0);
        assertTrue("and Coimbra", it.next().compareTo("Coimbra") == 0);
        assertTrue("and Leiria", it.next().compareTo("Leiria") == 0);
        assertTrue("and Lisboa", it.next().compareTo("Lisboa") == 0);
