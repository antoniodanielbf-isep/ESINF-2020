...
public class HeapPriorityQueue<K, V> extends AbstractPriorityQueue<K, V> {
    ...
    public List<V> getElemsLevel(int level){
        HeapPriorityQueue<K, V> newHeap = this.clone();
        int currentLevel = 1;
        List<V> l = new ArrayList<>();

        for(int i = 0; i < newHeap.size(); i = ((i*2)+1)){
            if(currentLevel == level){
                int aux =i;
                i = ((i*2)+1);
                for(int j = aux; j < i; j++){
                    l.add(newHeap.heap.get(j).getValue());
                }
                break;
            }
            currentLevel++;
    ...
    public List<V> getElemsLevel2(int level){
        List<V> allElems = new ArrayList<>();
        int minElementIndex = (int) Math.pow(2, level - 1) - 1;
        int maxElementIndex = (int) Math.pow(2, level) - 1;
        for (int x = minElementIndex; x < maxElementIndex && x < size(); x++){
            allElems.add(heap.get(x).getValue());
        }
        return allElems;
    }
