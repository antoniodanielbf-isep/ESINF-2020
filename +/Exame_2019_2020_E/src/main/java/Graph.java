...
public class Graph<V,E> implements GraphInterface<V,E> {
    ...
    public Iterable<Edge<V,E>> edges() {
        ...
        for(Vertex<V,E> vertex: vertices.values()) {
            edges.addAll((Collection<? extends Edge<V, E>>) vertex.getAllOutEdges());
        }
        return edges;
    }
    ...
    public V[] endVertices(gEdge<V,E> edge){
    ...
    public boolean removeVertex(V vert){
        ...
        for (graphbase.Edge<V,E> edge : incomingEdges(vert)){
            V vadj = edge.getVOrig();
            removeEdge(vadj,vert);
        }
        
        graphbase.Vertex<V,E> vertex = vertices.get(vert);
        
        //update the keys of subsequent vertices in the map
        for (graphbase.Vertex<V,E> v : vertices.values()){
    ...
    public boolean removeEdge(V vOrig, V vDest) {
        ...
        graphbase.Edge<V,E> edge = getEdge(vOrig,vDest);
        
        if (edge == null)
          return false;
        
        graphbase.Vertex<V,E> vorig = vertices.get(vOrig);
        ...
        if (!isDirected){ 
            ...
            if (edge != null){
               graphbase.Vertex<V,E> vdest = vertices.get(vDest);
    ...
    public Graph<V,E> clone() {
        ...
        for (V vert1 : vertices.keySet()) 
            for (graphbase.Edge<V,E> e : this.outgoingEdges(vert1))
                if (e != null){
    ...
    public V[] allkeyVerts() {
        ...
        V[] keyverts = (V [])
                Array.newInstance(vertices.keySet().iterator().next().getClass(), numVert);

        for (graphbase.Vertex<V,E> vert : vertices.values())
