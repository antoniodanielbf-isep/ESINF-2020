...
public class HeapPriorityQueue<K, V> extends AbstractPriorityQueue<K, V> {
    ...
    public List<V> getElemsLevel(int level) {
        List<V> allElements = new ArrayList<V>();
        int minElement = (int) Math.pow(2, level - 1) - 1;
        int elementNextToMax = (int) Math.pow(2, level) - 1;

        for (int i = minElement; i < elementNextToMax; i++) {
            allElements.add(heap.get(i).getValue());
        }
        return allElements;
    }
